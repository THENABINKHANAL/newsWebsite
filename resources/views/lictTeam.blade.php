...
<div class="lict-team-content">
    ...
    <h1 style="text-align:center; margin-top:0px;">Advisors</h1>
    <div class="members">
        @foreach ($members as $member)
            
    <div style = "color:inherit; text-decoration:none;" class="team-member" 
    @if($member['prvProfile']=="")
    onclick="location.href='user/{{$member['id']}}'"
    @else
    onclick="location.href='{{$member['prvProfile']}}'"
    @endif>
...
<div class="lict-team-content">
    <h1 style="text-align:center;">Core Research and Development Team</h1>
    <div class="members">
            @foreach ($coremembers as $member)
                
        <div style = "color:inherit; text-decoration:none;z-index:50;" class="team-member" 
        @if($member['prvProfile']=="")
        onclick="location.href='user/{{$member['id']}}'"
        @else
        onclick="location.href='{{$member['prvProfile']}}'"
        ...
        >
            ...
            <div class="member-info">
                ...
                <p class="lict-title" >{{$member['lictTitle']}}</p>
                @if($member['imgLoc']=="")
                <img id="imgLoc" src="{{url('img/deafultuserimgblack.png')}}"/>
                @else
                <img id="imgLoc" src="{{$member['imgLoc']}}"/>
                ...
                <a style="color:blue;z-index:100;text-align:center;" href='{{$member['publications']}}'>{{$member['publications']}}
                </a>
                @endif
                @if($member['researchAreas']!="")
                <p>Research Areas: {{$member['researchAreas']}}</p>
